...
public final class ParamUtils {
     ...
     * @return
     */
    public static boolean checkBluenoteId(String bluenoteId) {
        // 检查长度
        if (bluenoteId.length() < ID_MIN_LENGTH || bluenoteId.length() > ID_MAX_LENGTH) {
            return false;
        }
        // 检查格式
        Pattern pattern = Pattern.compile(ID_REGEX);
        return pattern.matcher(bluenoteId).matches();
    }
